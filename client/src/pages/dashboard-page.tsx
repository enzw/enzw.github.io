...
import { ArrowDownLeft, ArrowRight, ArrowRightLeft, ArrowUpRight, CalendarDays, CircleDollarSign, PiggyBank, ReceiptText, Sparkles, Target, WalletCards } from "lucide-react"
...
import type { DashboardData, TransactionHistoryItem } from "@/types/finance"
...
function recentActivityName(item: TransactionHistoryItem) {
  if (item.type === "TRANSFER") {
    return item.note || "Pindah saldo"
  }
  return item.note || item.category?.name || item.savingGoal?.name || labels[item.type]
}

function recentActivityWallet(item: TransactionHistoryItem) {
  return item.type === "TRANSFER"
    ? `${item.fromWallet.name} ke ${item.toWallet.name}`
    : item.wallet.name
}

function recentActivityPrefix(item: TransactionHistoryItem) {
